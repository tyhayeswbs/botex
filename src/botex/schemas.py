...
def create_answers_response_model(questions_json):
    ...
    for id_, question in questions_json.items():
        ...
        if qtype in ['text', 'textarea', 'str']:
            ...
            raise ValueError(f"Unsupported question type: {qtype}. At the moment botex only supports 'text', 'textarea', 'float', 'number', 'radio', 'select-one', and 'button-radio' question types. Please consider raising an issue on the GitHub repository. https://github.com/trr266/botex/issues")
...
def create_answers_response_model_no_mem(questions_json):
    answer_fields = {}
    for id_, question in questions_json.items():
        qlabel = question['question_label']
        qtype = question['question_type']
